...
module.exports.DriverOrderNoAssign = async (req, res) => {
    try {
        ...
        // const acceptedOrders = await DriverAcceptReject.findAll({
        ...
        res.json({ Till: orderTill, count: count, order: rows})
    }
    catch (error) {
        res.status(400).json({
            message: error.message
...
module.exports.DriverOrderCancell = async (req, res) => {
    try {
        ...
        res.json({ msg: cancelled, data: "Order Cancelled Sucessfully" });
    } catch (error) {
        console.error(error);
        return res.status(500).json({ message: "Server error" });
    }
